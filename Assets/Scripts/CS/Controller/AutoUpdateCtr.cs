...
public class AutoUpdateCtr : FormCtr
{
    ...
    private IEnumerator DownloadAsync(string serverVersion)
    {
        ...
        // 创建临时缓存目录
        string tempPath = Ap.Base.Environment.TempPath + "/" + serverVersion;
        ...
        if (File.Exists(path))
        {
            switch (info.ResType)
            {
                case ResTypeEnum.AssetBundle:
                    {
                        string toPath = Ap.Base.Environment.AssetBundleUpdatePath + "/" + info.Path;
                        File.Copy(path, toPath, true);
                    }
                    break;
                case ResTypeEnum.LuaScript:
                    {
                        string toPath = Ap.Base.Environment.LuaPath + "/" + info.Path;
                        File.Copy(path, toPath, true);
                    }
                    break;
            }
